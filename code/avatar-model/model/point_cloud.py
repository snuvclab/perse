...
class PointCloud(nn.Module):
    ...
    def upsample_points(self, new_points):
        self.points = nn.Parameter(torch.cat([self.points.to(new_points.device), new_points], dim=0))
    ...
    def upsample_130000_points(self, new_points):
        num_points = self.points.shape[0]
        num_upsample = 130000 - num_points
        if num_upsample == 0:
            self.points = nn.Parameter(self.points)
        else:
            rnd_idx = torch.randint(0, new_points.shape[0], (num_upsample,))
